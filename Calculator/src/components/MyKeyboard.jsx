...
export default function MyKeyboard() {
  ...
  const handleOperationPress = (buttonValue) => {
    if (buttonValue === "+/-") {
      ...
      setFirstNumber(firstNumber.charAt(0) === "-" ? firstNumber.slice(1) : `-${firstNumber}`);
    } else if (buttonValue === "％") {
      // Handle percentage operation
      if (operation === "+" || operation === "-") {
        // Percentage applied to the second number for addition/subtraction
        const percentageValue = parseFloat(firstNumber) / 100;
        const newValue = operation === "+"
          ? parseFloat(secondNumber) + parseFloat(secondNumber) * percentageValue
          : parseFloat(secondNumber) - parseFloat(secondNumber) * percentageValue;
        setResult(newValue);
        setFirstNumber("");
      } else if (operation === "*") {
        // For multiplication, calculate percentage as multiplier
        const percentageValue = parseFloat(firstNumber) / 100;
        const newValue = parseFloat(secondNumber) * percentageValue;
        setResult(newValue);
        setFirstNumber("");
      }
    } else {
